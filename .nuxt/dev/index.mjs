...
import { defineEventHandler, handleCacheHeaders, splitCookiesString, createEvent, fetchWithEvent, isEvent, eventHandler, setHeaders, sendRedirect, proxyRequest, getRequestHeader, setResponseHeaders, setResponseStatus, send, getRequestHeaders, setResponseHeader, appendResponseHeader, getRequestURL, getResponseHeader, removeResponseHeader, createError, getQuery as getQuery$1, readBody, createApp, createRouter as createRouter$1, toNodeListener, lazyEventHandler, getResponseStatus, getRouterParam, getResponseStatusText } from 'file://C:/APP/BDDB/node_modules/h3/dist/index.mjs';
import { escapeHtml } from 'file://C:/APP/BDDB/node_modules/@vue/shared/dist/shared.cjs.js';
...
import unstorage_47drivers_47fs from 'file://C:/APP/BDDB/node_modules/unstorage/drivers/fs.mjs';
import { digest } from 'file://C:/APP/BDDB/node_modules/nitropack/node_modules/ohash/dist/index.mjs';
import { toRouteMatcher, createRouter } from 'file://C:/APP/BDDB/node_modules/radix3/dist/index.mjs';
import { readFile } from 'node:fs/promises';
import consola, { consola as consola$1 } from 'file://C:/APP/BDDB/node_modules/consola/dist/index.mjs';
import { ErrorParser } from 'file://C:/APP/BDDB/node_modules/youch-core/build/index.js';
import { Youch } from 'file://C:/APP/BDDB/node_modules/youch/build/index.js';
import { SourceMapConsumer } from 'file://C:/APP/BDDB/node_modules/source-map/source-map.js';
...
import Datastore from 'file://C:/APP/BDDB/node_modules/@seald-io/nedb/index.js';
import { promises } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { dirname as dirname$1, resolve as resolve$1 } from 'file://C:/APP/BDDB/node_modules/pathe/dist/index.mjs';
import { walkResolver } from 'file://C:/APP/BDDB/node_modules/unhead/dist/utils.mjs';

const serverAssets = [{"baseName":"server","dir":"C:/APP/BDDB/server/assets"}];
...
storage.mount('root', unstorage_47drivers_47fs({"driver":"fs","readOnly":true,"base":"C:/APP/BDDB","watchOptions":{"ignored":[null]}}));
storage.mount('src', unstorage_47drivers_47fs({"driver":"fs","readOnly":true,"base":"C:/APP/BDDB/server","watchOptions":{"ignored":[null]}}));
storage.mount('build', unstorage_47drivers_47fs({"driver":"fs","readOnly":false,"base":"C:/APP/BDDB/.nuxt"}));
storage.mount('cache', unstorage_47drivers_47fs({"driver":"fs","readOnly":false,"base":"C:/APP/BDDB/.nuxt/cache"}));
storage.mount('data', unstorage_47drivers_47fs({"driver":"fs","base":"C:/APP/BDDB/.data/kv"}));
...
const _VQII475m8L5RGf7SMnwOXwxNehbvcOYb0DvZ2uZz00 = (function(nitro) {
  nitro.hooks.hook("render:html", (htmlContext) => {
    htmlContext.head.push(`<script>${script}<\/script>`);
  });
});

const rootDir = "C:/APP/BDDB";
...
const _JHsvEiTDHhjNBlpMZmBeAMzuH2E4tkFiE3nqNUpj78 = (nitroApp) => {
...
function getDb(collection) {
  if (!databases[collection]) {
    ...
    databases[collection].setAutocompactionInterval(1e3);
  }
...
async function addTorrent(data) {
  const doc = { ...data, synced_at: now() };
  return new Promise((resolve, reject) => {
    getDb("torrents").findOne({ hash: data.hash }, (err, existing) => {
      if (err) return reject(err);
      existing ? getDb("torrents").update({ hash: data.hash }, { $set: doc }, {}, (err2) => err2 ? reject(err2) : resolve()) : getDb("torrents").insert(doc, (err2) => err2 ? reject(err2) : resolve());
    });
  });
}
...
async function saveVolume(torrentId, files, data) {
  const doc = {
  ...
  return new Promise((resolve, reject) => {
    getDb("volumes").findOne({ torrent_id: torrentId, volume_no: data.volume_no }, (err, existing) => {
      if (err) return reject(err);
      existing ? getDb("volumes").update({ torrent_id: torrentId, volume_no: data.volume_no }, { $set: doc }, {}, (err2) => err2 ? reject(err2) : resolve()) : getDb("volumes").insert({ ...doc, created_at: now() }, (err2) => err2 ? reject(err2) : resolve());
    });
  });
}
function saveTorrentFiles(torrentId, files) {
  return new Promise((resolve, reject) => {
    ...
    const docs = files.map((f) => {
      ...
      return {
        ...f,
        torrent_id: torrentId,
        is_deleted: (_a = f.is_deleted) != null ? _a : false,
        synced_at: (_b = f.synced_at) != null ? _b : nowTs
      };
    });
    db.remove({ torrent_id: torrentId }, { multi: true }, (err) => {
      if (err) return reject(err);
      if (docs.length === 0) return resolve();
      db.insert(docs, (err2) => err2 ? reject(err2) : resolve());
    });
  });
}
...
function softDeleteTorrentFiles(torrentId) {
  return new Promise((resolve, reject) => {
    getDb("files").update(
      { torrent_id: torrentId },
...
const plugins = [
  _VQII475m8L5RGf7SMnwOXwxNehbvcOYb0DvZ2uZz00,
_JHsvEiTDHhjNBlpMZmBeAMzuH2E4tkFiE3nqNUpj78,
...
const _KEecAP = eventHandler((event) => {
...
const getServerEntry = () => import('file://C:/APP/BDDB/.nuxt//dist/server/server.mjs').then((r) => r.default || r);
// @ts-expect-error file will be produced after app build
const getClientManifest = () => import('file://C:/APP/BDDB/.nuxt//dist/server/client.manifest.mjs').then((r) => r.default || r).then((r) => typeof r === "function" ? r() : r);
// -- SSR Renderer --
...
const _lazy_UxiCwG = () => Promise.resolve().then(function () { return index_get$1; });
const _lazy_DUW4ey = () => Promise.resolve().then(function () { return index_post$1; });
const _lazy_vPV4vc = () => Promise.resolve().then(function () { return renderer$1; });

const handlers = [
  { route: '', handler: _KEecAP, lazy: false, middleware: true, method: undefined },
  { route: '/api/qb/torrents/delete', handler: _lazy_e9oMGf, lazy: true, middleware: false, method: "post" },
  { route: '/api/qb/torrents/files', handler: _lazy_RHU9yM, lazy: true, middleware: false, method: "get" },
  { route: '/api/qb/torrents/info', handler: _lazy_tGkfwI, lazy: true, middleware: false, method: "get" },
  { route: '/api/qb/torrents/stats', handler: _lazy_jmXrn1, lazy: true, middleware: false, method: "get" },
  ...
  { route: '/api/volumes', handler: _lazy_UxiCwG, lazy: true, middleware: false, method: "get" },
  { route: '/api/volumes', handler: _lazy_DUW4ey, lazy: true, middleware: false, method: "post" },
  { route: '/__nuxt_error', handler: _lazy_vPV4vc, lazy: true, middleware: false, method: undefined },
  { route: '/__nuxt_island/**', handler: _SxA8c9, lazy: false, middleware: false, method: undefined },
  { route: '/**', handler: _lazy_vPV4vc, lazy: true, middleware: false, method: undefined }
];
...
const delete_post = defineEventHandler(async (event) => {
  const hash = getQuery$1(event).hash;
    ...
    for (const t of torrents) {
      ...
      if (!exists) {
        try {
          ...
          const files = qbFiles.map((f) => ({
            ...f,
            is_deleted: false
          }));
          await addTorrent({ ...data, is_deleted: false });
...
const files_get$2 = defineEventHandler(async (event) => {
  ...
  const files = ((_a = qbFiles[0]) == null ? void 0 : _a.files) || [];
...
const info_get = defineEventHandler(async (event) => {
  const { state, search, hash } = getQuery$1(event);
  if (hash) {
    const torrent = await getTorrent(hash);
  ...
  if (state) {
    ...
    torrents = torrents.filter((t) => {
      var _a;
      if (s === "paused") return (_a = t.state) == null ? void 0 : _a.includes("paused");
...
const sync_post = defineEventHandler(async () => {
  ...
  return result.success ? { success: true, data: "synced" } : { success: false, error: result.error };
});
...
const bdInfo_get = defineEventHandler(async (event) => {
  const { torrent_id, torrent_file_id } = getQuery$1(event);
...
const bdInfo_get$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: bdInfo_get
}, Symbol.toStringTag, { value: 'Module' }));

const bdInfo_post = defineEventHandler(async (event) => {
  const { torrent_id } = getQuery$1(event);
...
const bdInfo_post$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: bdInfo_post
}, Symbol.toStringTag, { value: 'Module' }));
...
const index_get = defineEventHandler(async (event) => {
  const { torrent_id, torrent_file_id } = getQuery$1(event);
  const volumes = torrent_id ? await getVolumesByTorrent(torrent_id) : torrent_file_id ? await getVolumesByFile(torrent_file_id) : await getAllVolumes();
  return { success: true, data: JSON.stringify(volumes) };
});

...
const computeNodeMediaValue = (
    key: string,
    flatTree: FlatTree,
    dataMap: Map<string, NodeData>,
): { media_no: number | undefined; isConsistent: boolean } => {
    const node = flatTree.map.get(key);
    if (!node || node.isLeaf) {
        return {
            media_no: dataMap.get(key)?.media_no,
            isConsistent: true,
        };
    }

    const childValues = node.children.map((childKey) =>
...
const clearInconsistentParentAssignments = (
    ...
    const nextMap = new Map(dataMap);
    flatTree.order.forEach((key) => {
        const node = flatTree.map.get(key);
        if (!node || node.isLeaf || node.children.length === 0) return;

        const computed = computeNodeMediaValue(key, flatTree, nextMap);
        if (computed.isConsistent) return;

        const current = nextMap.get(key);
        if (!current) return;
        if (current.media_no === undefined && !current.shared_medias?.length)
            return;

        nextMap.set(key, {
            ...current,
            media_no: undefined,
            shared_medias: undefined,
        });
    });
...
export function useMediaEditor(onSave?: () => void): UseMediaEditorReturn {
    ...
    const [flatTree, setFlatTree] = useState<FlatTree>({
        map: new Map(),
        order: [],
        leaves: [],
    });
    const [defaultExpandedKeys, setDefaultExpandedKeys] = useState<string[]>([]);
    const [mediaToKeys, setMediaToKeys] = useState<Map<number, Set<string>>>(
        new Map(),
    );
    ...
    const hasChanges = useCallback((): boolean => {
        for (const [key, data] of nodeDataRef.current.entries()) {
            if (
                (data.media_no ?? undefined) !==
                (initialNodeDataRef.current.get(key)?.media_no ?? undefined) ||
                JSON.stringify(data.shared_medias) !==
                JSON.stringify(initialNodeDataRef.current.get(key)?.shared_medias)
            )
                return true;
        }
        return (
            JSON.stringify(mediaFormsRef.current) !==
            JSON.stringify(initialMediaFormsRef.current)
        );
    }, []);
    ...
    const getComputedNodeValue = useCallback(
        (
            key: string,
        ): {
            media_no: number | undefined;
            isConsistent: boolean;
        } => computeNodeMediaValue(key, flatTree, nodeData),
        [flatTree, nodeData],
    );

    const selectedMedias = useMemo(
        () => Array.from(mediaToKeys.keys()).sort((a, b) => a - b),
        [mediaToKeys],
    );

    const getNodeMediaNo = useCallback(
        (key: string) => nodeData.get(key)?.media_no,
        [nodeData],
    );
    const getNodeShared = useCallback(
        (key: string) => Array.isArray(nodeData.get(key)?.shared_medias),
        [nodeData],
    );
    ...
    const getAllChildrenKeys = useCallback(
        (key: string) => flatTree.order.filter((k) => k.startsWith(key + "/")),
        [flatTree],
    );
    ...
    const onToggleShared = useCallback(
        (key: string, shared: boolean) => {
            ...
            setNodeData(normalizedMap);
            setMediaToKeys(newMtK);
        },
        [getAllChildrenKeys, nodeData, mediaToKeys, flatTree],
    );

    const onSharedMediaChange = useCallback(
        (key: string, medias: number[]) => {
            const nodes = [key, ...getAllChildrenKeys(key)];
            const newMap = new Map(nodeData);
            const newMtK = new Map(mediaToKeys);
    ...
    const onMediaNoChange = useCallback(
        (key: string, mediaNo: number | null) => {
            ...
            const newMtK = new Map(mediaToKeys);
            nodes.forEach((k) => {
                const oldNo = nodeData.get(k)?.media_no;
                if (no !== undefined) {
    ...
    const open = useCallback(
        async (volumeId: string, volumeNo?: number, catalogNo?: string) => {
            ...
            try {
                const filesResult = await fetchApi<FileItem[]>(
                    `/api/volumes/${volumeId}/files`,
                );
                ...
                const mediaResult = await fetchApi<Media[]>(
                    `/api/volumes/${volumeId}/medias`,
                );
                    ...
                    if (medias.length > 0) {
                        ...
                        fileToKeyMap.forEach((key, fid) => {
                            ...
                            if (mediaNos.length === 1) {
                                ...
                                if (!newMtK.has(mediaNos[0]))
                                    newMtK.set(mediaNos[0], new Set());
                                newMtK.get(mediaNos[0])!.add(key);
                            } else {
                                newND.set(key, {
                                    ...ex,
                                    media_no: undefined,
                                    shared_medias: mediaNos,
                                });
                                mediaNos.forEach((m) => {
                                    if (!newMtK.has(m)) newMtK.set(m, new Set());
                        ...
                        const sorted = Array.from(ft.order).sort(
                            (a, b) =>
                                (ft.map.get(b)?.depth ?? 0) - (ft.map.get(a)?.depth ?? 0),
                        );
                        sorted.forEach((key) => {
                            ...
                            const inter = sets.reduce(
                                (acc, s) => new Set([...acc].filter((v) => s.has(v))),
                            );
    ...
    const handleSubmit = useCallback(async (): Promise<boolean> => {
        ...
        setSubmitted(true);
        const hasError = selectedMedias.some(
            (m) => !mediaForms[m]?.content_title?.trim(),
        );
        if (hasError) {
        ...
        try {
            ...
            const result = await postApi(
                "/api/volumes/" + volumeInfo.volumeId + "/medias",
                {
                ...
                {selectedMedias.map((no) => {
                    ...
                    return (
                        <Space key={no} style={{ width: "100%" }} size={8}>
                            ...
                            <Typography.Text>
                                {form.content_title?.trim() ? form.content_title : "—"}
                            </Typography.Text>

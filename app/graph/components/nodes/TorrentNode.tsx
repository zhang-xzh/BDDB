...
export const TorrentNode: React.FC<NodeProps> = memo(function TorrentNode({ data }) {
  const { torrent } = data as unknown as TorrentNodeData;
  const { token } = theme.useToken();

  return (
    <Card size="small" style={{ width: 220, borderLeft: `4px solid ${token.colorPrimary}`, background: token.colorPrimaryBg }} styles={{ body: { padding: 12 } }}>
